...
try:
    ...
    result = BRUCE_WILLIS * leeloo
    print(f'- Leeloo Dallas! Multi-pass № {result}!')
except ValueError as temp:
    print(type(temp), '- невозможно преобразовать к числу')
except IndexError as temp:
    print(type(temp), '- выход за границы списка')
except Exception as temp:
    print(type(temp), 'поймано исключение')
